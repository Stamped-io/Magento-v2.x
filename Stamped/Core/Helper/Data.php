...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const STAMPED_SECURED_API_URL = "https://%s:%s@stamped.io/api/v2/%s";
    /**
     ...
     */
    public function __construct(\Magento\Framework\App\Helper\Context $context, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\Framework\Filesystem $filesystem, \Magento\Framework\File\Size $fileSize, \Magento\Framework\HTTP\Adapter\FileTransferFactory $httpFactory, \Magento\Catalog\Api\ProductRepositoryInterface $productRepository, \Magento\MediaStorage\Model\File\UploaderFactory $fileUploaderFactory, \Magento\Framework\Filesystem\Io\File $ioFile, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\Image\Factory $imageFactory, \Magento\Catalog\Helper\Image $imgHelper, \Magento\Framework\Escaper $escaper, \Magento\Store\Model\App\Emulation $appEmulation)
    {
    ...
    public function getConfig($config_path)
    {
        return $this
            ->scopeConfig
            ->getValue($config_path, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
    }
    ...
    public function API_POST($path, $data, $store, $timeout = 30)
    {
        try
        {
            //'https://requestbinstamped.herokuapp.com/1c5bf1i1'
    ...
    public function getApiStoreUrl($store)
    {
        ...
        if (!$store_url)
        {
            $store_url = Mage::app()->getStore($store->getId())
                ->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK);
        }
        return $store_url;
    }
    
    public function getApiStoreHash($store)
    {        
        return $this->getConfigValue(self::STAMPED_STORE_HASH_CONFIGURATION, $store);
    }
    ...
    public function getApiUrlAuth($store)
    {
        ...
        $store_hash = $this->getApiStoreHash($store);

        return sprintf(self::STAMPED_SECURED_API_URL, $apiKey, $apiSecret, $store_hash);
    }
    ...
    public function createRewardRequest($order, $store)
    {
        return $this->API_POST("/orders", $order, $store);
    }
    ...
    public function saveOrderAfter($order)
    {
        try
        {
            ...
            $isProceedReview = false;
            $isProceedReward = false;
            ...
            $orderStatusesRewards = $this->getConfigValue('stamped_core/stamped_settings_rewards/order_status_trigger_rewards');
            if ($orderStatusesRewards == null)
            {
                $orderStatusesRewards = array(
                    'processing'
                );
            }
            else
            {
                $orderStatusesRewards = array_map('strtolower', explode(',', $orderStatusesRewards));
            }
            ...
            {
                $isProceedReview = true;
            }
            
            if (in_array($order->getStatus(), $orderStatusesRewards))
            {
                $isProceedReward = true;
            }
            ...
            $data["orderTotal"] = $order->getGrandTotal();
            $data["orderStatus"] = strtolower($order->getStatus());
            ...
            if ($isProceedReward){
                $this->createRewardRequest($data, $store_id);
			}
